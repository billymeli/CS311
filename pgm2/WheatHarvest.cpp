...
int main(void) {
  ...
  for (int i = 0; i < tickets.size(); i++) {
    cout << "\nTicket " << tickets[i].getTicketNumber() << ":" << endl;
    cout << "\t" << tickets[i].getGrossWeight() << " Gross Weight" << endl;
    cout << "\t" << tickets[i].getTareWeight() << " Tare Weight" << endl;
    cout << "\t" << tickets[i].calcNetWeight() << " Net Weight\n" << endl;
    cout << "\t" << tickets[i].calcGrossBushels() << " Gross Bushels" << endl;
    cout << "\t" << tickets[i].calcMoistureDockage() << " Moisture Level" << "(" << tickets[i].getMoistureLevel() << "%)" << endl;
    cout << "\t" << tickets[i].calcForeignMaterialDockage() << " Foreign Material" << "(" << tickets[i].getForeignMaterial() << "%)" << endl;
    cout << "\t" << tickets[i].calcNetBushels() << " Net Bushels" << endl;
    totalNetBushels += tickets[i].calcNetBushels();
    totalGrossBushels += tickets[i].calcGrossBushels();
  }

  //Printing the harvest summary
  cout << "Wheat harvest summary" << endl;
